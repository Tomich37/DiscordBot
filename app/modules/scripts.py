...
class Scripts:
    ...
    def _convert_videos_in_dir(self, video_folder: Path) -> list[Path]:
        converted_files = []
        video_files = [
            file
            for file in video_folder.iterdir()
            if file.is_file() and file.suffix.lower() in {".mp4", ".avi", ".mkv", ".mov", ".webm"}
        ]
    ...
    async def _send_converted_files(self, inter, converted_files: list[Path]):
        files = [disnake.File(str(file_path)) for file_path in converted_files]
        await inter.followup.send(
            content=f"{inter.author.mention}, конвертация завершена. Готовые файлы ниже.",
            files=files,
        )

    async def process_video_conversion(self, inter, attachments):
        if not attachments:
        ...
        try:
            if was_queued:
                await inter.followup.send(
                    f"{inter.author.mention}, запрос поставлен в очередь. "
                    f"Позиция в очереди: {queue_position}."
                )
            else:
                await inter.followup.send(
                    f"{inter.author.mention}, запрос принят в обработку."
                )
            ...
            converted_files = await asyncio.to_thread(self._convert_videos_in_dir, job_dir)

            if not converted_files:
                await inter.followup.send(
                    f"{inter.author.mention}, не удалось сконвертировать вложения. "
                    "Поддерживаются файлы `.mp4`, `.avi`, `.mkv`, `.mov`, `.webm`."
                )
                return

            await self._send_converted_files(inter, converted_files)
        except Exception as e:
